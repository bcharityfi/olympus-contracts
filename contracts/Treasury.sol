...
contract OlygiveTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SOGV }
    ...
    address public sOGV;
    uint public sOGVQueue; // Delays change to sOGV address
    ...
    constructor (
       ...
       isLiquidityToken[ _OGVDAI ] = true;
       liquidityTokens.push( _OGVDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for OGV
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint OGV needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( OGV ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn OGV for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOGVERC20( OGV ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sOGV ).balanceOf( msg.sender ); // Can only borrow against sOGV held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with OGV
        @param _amount uint
     */
    function repayDebtWithOGV( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IOGVERC20( OGV ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, OGV, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( OGV ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( OGV ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns OGV valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match OGV decimals
            value_ = _amount.mul( 10 ** IERC20( OGV ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SOGV ) { // 9
            sOGVQueue = block.number.add( blocksNeededForQueue );
        } else return false;
