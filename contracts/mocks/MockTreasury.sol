...
contract MockOlygiveTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sOGV;
  uint256 public sOGVQueue; // Delays change to sOGV address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sOGV).balanceOf(msg.sender); // Can only borrow against sOGV held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithOGV(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(OGV).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SOGV) {
      // 9
